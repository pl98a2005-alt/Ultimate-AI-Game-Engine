...
# إعداد الخط لضمان عدم ظهور مربعات [cite: 2026-01-11]
FONT_PATH = "font.ttf"
...
class SovereignEngineer(App):
    def build(self):
        ...
        self.blueprint_input = TextInput(
            hint_text="صف اللعبة التي تريدها هنا بالكامل...",
        ...
        build_btn = Button(text="بدء الهندسة والحقن", font_name=FONT_PATH if os.path.exists(FONT_PATH) else None, background_color=get_color_from_hex('#004466'), bold=True)
        build_btn.bind(on_press=self.run_engine)
        
        clean_btn = Button(text="تنظيف", font_name=FONT_PATH if os.path.exists(FONT_PATH) else None, background_color=get_color_from_hex('#660000'), size_hint_x=0.3)
        clean_btn.bind(on_press=self.smart_cleanup)

        controls.add_widget(build_btn)
        controls.add_widget(clean_btn)
        self.root_layout.add_widget(self.header); self.root_layout.add_widget(self.console_scroll)
        self.root_layout.add_widget(self.blueprint_input); self.root_layout.add_widget(controls)
    ...
    def run_engine(self, instance):
        if self.blueprint_input.text:
            threading.Thread(target=self.process_engineering).start()

    def process_engineering(self):
        self.console_output.text += "\n>> جاري البحث والهندسة الذكية..."
        time.sleep(2)
        self.console_output.text += "\n>> تم حقن الأكواد والأصول بنجاح!"
        self.header.color = get_color_from_hex('#FFD700')

    def smart_cleanup(self, instance):
        self.blueprint_input.text = ""
        self.console_output.text = ">> تم التنظيف مع حفظ الخبرة." [cite: 2026-01-13]
    def ensure_font_exists(self):
